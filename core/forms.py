...
class StudentSignUpForm(UserCreationForm):
    courses = (
        ("BCA", "Bachelors In Computer Applications (Computer Science)"),
        ("MCA", "Masters In Computer Applications (Computer Science)"),
    )
...
class CreateProjectForm(forms.Form):
    title = forms.CharField(
        max_length=100, required=True, help_text="Enter your Project Title"
    )
    abstract = forms.CharField(
        widget=forms.Textarea(
            attrs={"placeholder": "Type in your project abstract here"}
        ),
        min_length=200,
        max_length=1000,
        required=True,
    ...
    abstract_document = forms.FileField(
        help_text="Upload your abstract document in a doc or docx format"
    )
...
class SendCommentForm(forms.Form):
    comment = forms.CharField(
        ...
        initial="Asas",
        widget=forms.Textarea(
            attrs={"rows": "2", "cols": "30", "placeholder": "Add your comment here"}
        ),
    )
...
class UpdateProfileForm(forms.Form):
    user = User
    first_name = forms.CharField(
        initial="supervisor", max_length=50, widget=forms.TextInput()
    )
...
class TestForm(forms.Form):
    phone_numbers = forms.CharField(max_length=100, widget=forms.Textarea())
