...
urlpatterns = [
    ...
    path(
        "appointments/delete_timeslots(<availableday_id>[0-9]+)(<day>[a-z]+)",
        DeleteAvailableDays,
    ...
    path(
        ...
        view_approved_appointments,
        name="approved_appointments",
    ),
    path("appointments/view_available_slots", ViewAvailableDays, name="view_available"),
    path(
        "appointments/approve_appointment(<appointment_id>[0-9]+)",
        ApproveAppointment,
        name="approve_appointment",
    ),
    path(
        "appointments/reject_appointment(<appointment_id>[0-9]+)",
        RejectAppointment,
    ...
    path(
        "student/profile/update_profile", UpdateProfile, name="student_update_profile"
    ),
    ...
    path(
        "supervisor/profile/update_profile",
        UpdateProfile,
        name="supervisor_update_profile",
    ),
    path(
        "student/project/upload_document(<milestone_id>[0-9]+)",
        upload_file,
        name="upload_document",
    ),
    path("project/download(<document_id>[0-9]+)", download_document, name="download"),
    path(
        "project/download_abstract(<abstract_id>[0-9]+)",
        download_abstract,
        name="download_abstract",
    ),
    path(
        "project/view_comments(<milestone_id>[0-9]+)(<student_id>[0-9]+)",
        view_comment,
        name="view_comments",
    ),
    ...
    path(
        "supervisor/project_supervision(<student_id>[0-9]+)",
        project_supervision_view,
        name="project_supervision",
    ),
    path(
        "supervisor/project_supervision/close_milestone(<milestone_id>[0-9]+)(<project_id>[0-9]+)",
        CloseMilestone,
        name="close_milestone",
    ),
    path(
        "supervisor/project_supervision/close_project(<project_id>[0-9]+)",
        CloseProject,
        name="close_project",
    ),
